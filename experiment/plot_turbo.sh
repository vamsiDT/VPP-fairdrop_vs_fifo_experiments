...
plot "fair-drop_bell64_turbo_$BW.dat" using (column(0)):(f(\$1)):xtic(2) with linespoints lw 2 pt 7 t "VPP with Fairdrop module. $PACKETS_BELL packets captured in 1 sec","fair-drop_bell64_turbo_fifo_$BW.dat" using (column(0)-1):(f(\$1)):xtic(2) with linespoints lw 2 pt 5 t "VPP with Virtual FIFO $PACKETS_BELL_FIFO packets captured in 1 sec"
EOFMarker
