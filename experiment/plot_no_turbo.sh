...
plot "fair-drop_bell64_$BW.dat" using (column(0)):(f(\$1)):xtic(2) with linespoints lw 2 pt 7 t "VPP with Fairdrop module","fair-drop_bell64_fifo_$BW.dat" using (column(0)):(f(\$1)):xtic(2) with linespoints lw 2 pt 5 t "VPP with Virtual FIFO"
EOFMarker
...
plot "fair-drop_exponential64_$BW.dat" using (column(0)):(f(\$1)):xtic(2) with linespoints lw 2 pt 7 t "VPP with Fairdrop module","fair-drop_exponential64_fifo_$BW.dat" using (column(0)):(f(\$1)):xtic(2) with linespoints lw 2 pt 5 t "VPP with Virtual FIFO"
EOFMarker
